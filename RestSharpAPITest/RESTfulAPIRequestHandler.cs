...
[Obsolete]
        ...
        internal void SetupQueryParameters(Table table)
        {
            ...
            try
            {
                foreach (var row in table.Rows)
                {
                    ...
                    else if (value.Equals("GenerateUniqueKeycode"))
                    {
                        value = GenerateUniqueKeycode(20);
                    }

                    else if (value.Equals("GenerateUniqueDeviceID"))
                    {
                        value = GetUniqueDeviceID(30);
                    }
                    else if (value.Equals("GenerateUniqueInstanceID"))
                    {
                        value = GetUniqueInstanceID(30);
                    }
        ...
        internal void StoreDeviceinstanceIDIntoFile()
        {
            try
            {
                var rootPath = AppDomain.CurrentDomain.BaseDirectory;
                saveToFile += string.Format("deviceID:{0};instanceID:{1}", deviceIDValue, instanceIDValue) + Environment.NewLine;
                FileHandler.SaveToFile(rootPath, folderName, "deviceIDinstanceID.txt", saveToFile);
                ExtentReportsHandler.Pass("Saved deviceID to file deviceIDinstanceID.txt");
            }
            catch (Exception e)
            {
                ExtentReportsHandler.Fail("Error deviceID to file");
                Console.WriteLine(e);
              //  throw e;
            }
        }
        ...
        private string GetUniqueDeviceID(int length)
        {
            const string pool = "abcdefghijklmnopqrstuvwxyz0123456789";
            ...
            while (result.Length < length)
            {
                temp = result.Length == 2 ? "a" : pool[_random.Next(pool.Length)].ToString();

                result = result + temp;
            }

            StoreFeatureContextVariable("deviceId", result);
            deviceIDValue = result;
            return result;
        }

        private string GetUniqueInstanceID(int length)
        {
            const string pool = "abcdefghijklmnopqrstuvwxyz0123456789";
            ...
            while (result.Length < length)
            {
                temp = result.Length == 2 ? "b" : pool[_random.Next(pool.Length)].ToString();

                result = result + temp;
            }
            StoreFeatureContextVariable("instanceId", result);
            instanceIDValue = result;
            return result;
        }
